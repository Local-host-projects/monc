...
async function api(url,opt={}){let headers={Accept:'application/json',...(opt.body?{'Content-Type':'application/json'}:{}),...(opt.headers||{})};let r=await fetch(url,{...opt,headers});let j=await r.json().catch(()=>({detail:'Request failed'}));if(!r.ok){let e=Error(j.detail||'Request failed');e.status=r.status;throw e}return j}
...
function initShell(){setTheme(readTheme());$('.theme')?.addEventListener('click',()=>setTheme(document.documentElement.dataset.theme==='dark'?'light':'dark'));window.addEventListener('storage',e=>{if(e.key==='moncTheme')setTheme(e.newValue)});let c=$('.clock');if(c){let f=()=>c.textContent=new Date().toISOString().slice(11,19)+' UTC';f();setInterval(f,1000)}}
